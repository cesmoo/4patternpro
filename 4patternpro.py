...
async def init_db():
    try:
        ...
        print("🗄 MongoDB ချိတ်ဆက်မှု အောင်မြင်ပါသည်။ (🚀 Futuristic UI + Ultimate AI Edition)")
...
# ==========================================
# 🎨 5. DYNAMIC GRAPH GENERATOR (FUTURISTIC SCI-FI UI)
# ==========================================
def generate_winrate_chart(predictions):
    wins, losses = 0, 0
    history_wr, bar_colors_top, bar_colors_bottom, dots_list = [], [], [], []
    ...
    for p in latest_preds: 
        if 'WIN' in p.get('win_lose', ''):
            wins += 1
            bar_colors_top.append('#00ffcc')     # Neon Green Light
            bar_colors_bottom.append('#004d40')  # Dark Green Base
            dots_list.append(('G', '#00ffcc'))
        else:
            losses += 1
            bar_colors_top.append('#ff3333')     # Neon Red Light
            bar_colors_bottom.append('#4d0000')  # Dark Red Base
            dots_list.append(('R', '#ff3333'))
        total = wins + losses
    ...
    fig = plt.figure(figsize=(10.24, 7.68), facecolor='#111827') 
    
    # ---------------------------------------------------------
    # TOP SECTION: The Graph (Line & Gradient Bars)
    # ---------------------------------------------------------
    ...
    # Grid lines (Futuristic glowing grid)
    ax.grid(axis='y', color='#374151', linestyle='-', linewidth=1.5, alpha=0.7)
    ...
    ax.set_yticklabels(['0%', '25%', '50%', '75%', '100%'], color='#9ca3af', fontsize=14, fontweight='bold')
    ax.set_xticks([])
    
    if total_played > 0:
        x = np.arange(total_played)
        # Create gradient-like 3D Bars (Drawing two overlapping bars)
        ax.bar(x, [55]*total_played, color=bar_colors_bottom, width=0.75, bottom=0, edgecolor='black', linewidth=1)
        ax.bar(x, [15]*total_played, color=bar_colors_top, width=0.75, bottom=40, alpha=0.8) # Glowing top
        
        # The Line (Glowing Blue)
        ax.plot(x, history_wr, color='#3b82f6', linewidth=4, marker='o', markersize=8, markerfacecolor='#111827', markeredgecolor='#60a5fa', markeredgewidth=2.5, zorder=4)
    ...
    circle_bg = plt.Circle((0.5, 0.5), 0.45, color='#1f2937', ec='#3b82f6', lw=4, zorder=1)
    circle_ax.add_patch(circle_bg)
    circle_ax.text(0.5, 0.55, f"{win_rate}%", color='#00ffcc', fontsize=50, fontweight='bold', ha='center', va='center', zorder=2)
    circle_ax.text(0.5, 0.35, "CURRENT WINRATE", color='#9ca3af', fontsize=12, fontweight='bold', ha='center', va='center', zorder=2)

    # Left Box (WINS)
    win_box_ax = fig.add_axes([0.1, 0.25, 0.3, 0.15])
    win_box_ax.set_axis_off()
    win_box = patches.FancyBboxPatch((0,0), 1, 1, boxstyle="round,pad=0.1", fc="#064e3b", ec="#00ffcc", lw=2)
    win_box_ax.add_patch(win_box)
    win_box_ax.text(0.5, 0.5, f"WINS: {wins}", color='#00ffcc', fontsize=26, fontweight='bold', ha='center', va='center')

    # Right Box (LOSSES)
    lose_box_ax = fig.add_axes([0.6, 0.25, 0.3, 0.15])
    lose_box_ax.set_axis_off()
    lose_box = patches.FancyBboxPatch((0,0), 1, 1, boxstyle="round,pad=0.1", fc="#450a0a", ec="#ff3333", lw=2)
    lose_box_ax.add_patch(lose_box)
    lose_box_ax.text(0.5, 0.5, f"LOSSES: {losses}", color='#ff3333', fontsize=26, fontweight='bold', ha='center', va='center')

    # Prediction Count Box (Below Circle)
    count_box_ax = fig.add_axes([0.3, 0.13, 0.4, 0.08])
    count_box_ax.set_axis_off()
    count_box = patches.FancyBboxPatch((0,0), 1, 1, boxstyle="round,pad=0.1", fc="#1f2937", ec="#4b5563", lw=2)
    count_box_ax.add_patch(count_box)
    count_box_ax.text(0.5, 0.5, f"PREDICTION COUNT: {total_played}/20", color='#e5e7eb', fontsize=16, fontweight='bold', ha='center', va='center')

    # ---------------------------------------------------------
    # BOTTOM SECTION: Recent Predictions Nodes
    # ---------------------------------------------------------
    fig.text(0.5, 0.09, "Recent Predictions (Oldest ➔ Latest)", color='#9ca3af', fontsize=12, ha='center')

    if len(dots_list) > 0:
        dot_ax = fig.add_axes([0.1, 0.03, 0.8, 0.05]) 
        dot_ax.set_axis_off()
        ...
        for i, (char, color) in enumerate(dots_list):
            ...
            cy = 0.5
            # Draw Node Circle
            dot_ax.scatter(cx, cy, s=400, c=color, edgecolors='white', linewidths=1.5, zorder=5, alpha=0.9)
            # Add G or R text inside
            dot_ax.text(cx, cy, char, color='black', fontsize=12, fontweight='bold', ha='center', va='center', zorder=6)
            
    fig.text(0.5, 0.01, "DEV-WANG LIN  |  VER 2.1", color='#6b7280', fontsize=12, fontweight='bold', ha='center')

    # Save to Buffer
    buf = io.BytesIO()
    plt.savefig(buf, format='png', dpi=100, facecolor='#111827') # No bbox_inches tightly fixed to 1024x768
    buf.seek(0)
    plt.close(fig)
    return buf
...
async def main():
    print("🚀 Aiogram Bigwin Bot (Futuristic UI Edition) စတင်နေပါပြီ...\n")
